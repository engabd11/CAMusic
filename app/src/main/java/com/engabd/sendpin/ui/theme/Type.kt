 ...
 *
 * material3 1.4.0 stable (BOM 2026.06.01) does not expose the 30-param Typography
 * constructor — the emphasized parameters require material3 1.5.0-alpha26+ which
 * needs AGP 9.1 (the app stays on AGP 8.x). These styles are defined as plain
 * TextStyle values so they can be used directly via `style = HeadlineLargeEmphasized`
 * in any Text() call.
 *
 * When material3 1.5.0 goes stable with AGP 8.x support, fold these into
 * AppTypography and access via `MaterialTheme.typography.headlineLargeEmphasized`.
 *
