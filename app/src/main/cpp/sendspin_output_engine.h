 ...
 *
 * Why this exists: the previous Kotlin playback engine wrote each decoded PCM
 * chunk to an AudioTrack from a JVM thread, gating every chunk to its absolute
 * deadline (sleep-to-deadline). That kept the AudioTrack FIFO shallow, which
 ...
 *
 * Ported from Massdroid's `sendspin_output_engine.h`/`.cpp`
 * (github.com/sfortis/massdroid_native, MIT) with no functional changes — the
 * class was already generic C++/Oboe with no Massdroid-specific dependencies.
 *
...
public:
    ...
    // Surfaced for a sync-error UI; correction itself is internal.
    int64_t driftEmaUs() const { return driftEmaUs_.load(); }

    // Cumulative ring-underrun frames (callback ran dry within a buffer): the
    // real audible-dropout counter for a quality readout. 0 = clean.
    ...
    // the AudioManager device-callback never fires). The Kotlin side polls
