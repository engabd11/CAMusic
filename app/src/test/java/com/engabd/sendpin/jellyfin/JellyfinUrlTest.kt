...
class JellyfinUrlTest {
    ...
    /**
     * The default has to be the stored file. Omitting `static=true` lets the server
     * apply whatever transcoding profile it has for this client, which would silently
     * turn a FLAC into lossy AAC — the same trap `format=raw` avoids on Subsonic.
     */
    @Test
    fun `the default stream asks for the original file`() {
    ...
    @Test
    fun `a codec token asks for that container instead`() {
        val url = client("192.168.0.10:8096").streamUrl("song1", "flac")
        assertTrue("container=flac" in url, url)
        assertTrue("static=true" !in url, url)
    }
     ...
     */
    @Test
    fun `a bitrate in the token is converted to bits per second`() {
        val url = client("192.168.0.10:8096").streamUrl("song1", "mp3-320")
        assertTrue("container=mp3" in url, url)
    ...
    fun `a cover URL carries the requested width`() {
        assertEquals(
            "http://192.168.0.10:8096/Items/alb1/Images/Primary?maxWidth=500",
            client("192.168.0.10:8096").coverUrl("alb1", 500),
        )
    }
}
