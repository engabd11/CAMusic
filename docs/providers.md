...
| **Subsonic-compatible** | Same | Gonic, Airsonic, Astiga, Ampache, Funkwhale, epoupon's LMS. Same client; capabilities probed via `getOpenSubsonicExtensions`, so an older server loses lyrics rather than offering a pane it can't fill. Ampache ships with the API disabled until Preferences → System turns it on; Funkwhale issues its own Subsonic password rather than accepting the account one — both are named on the connect form. |
